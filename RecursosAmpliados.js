...
// @author missnoviembre.ogamegmail.com todos los creditos
// 
// ==/UserScript==
...
(function () {
    ...
    function ObjPlaneta() {
        ...
        this.getTotalM = function() {
            ...
            return base + mina + geo + ofi + plasma + amplificador;
        }
        ...
        this.getTotalD = function() {
            ...
            return mina + geo + plasma + ofi + amplificador - fusion;
        }
    ...
    function getDatosSummary() {
        ...
        if(tipo.indexOf("planet") != -1) {
			...
			// oficiales
            parcial = getContenido(lista, 13,2).innerHTML;
            parcial = parcial.substring(parcial.indexOf('">')+2, parcial.indexOf("</span>"));
            parcial = parcial.replace(/\./g, "").replace(/\,/g, "").trim();
			...
			// oficiales
            parcial = getContenido(lista, 13,3).innerHTML;
            parcial = parcial.substring(parcial.indexOf('">')+2, parcial.indexOf("</span>"));
            parcial = parcial.replace(/\./g, "").replace(/\,/g, "").trim();
			...
			// oficiales
            parcial = getContenido(lista, 13,4).innerHTML;
            parcial = parcial.substring(parcial.indexOf('">')+2, parcial.indexOf("</span>"));
            parcial = parcial.replace(/\./g, "").replace(/\,/g, "").trim();
            ...
            // almacen de metal
            parcial = getContenido(lista, 14,1).innerHTML;
            parcial = parcial.substring(parcial.indexOf('">')+2, parcial.indexOf("</span>"));
        ...
        if ( numPlanets > 0 ) { 
            ...
            for(var k = 0; k < sep.length; k++){
                if(sep[k].length > 3) {
                    ...
                    totalM = baseM + minaM + geoM + ofiM + plasmaM + amplificadoresM;
                    totalC = baseC + minaC + geoC + ofiC + plasmaC + amplificadoresC;
                    totalD = baseD + minaD + geoD + ofiD + plasmaD + amplificadoresD - gastoFusion;
